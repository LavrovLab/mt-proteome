...
# Not all mitocarta genes are present in txdb, hence the hassle with %in%
hc_mc_tx_by_gene <- transcriptsBy(txdb, "gene")[as.character(hs_mitocarta_gn[hs_mitocarta_gn %in% hs_all_gn_gr$gene_id])] ## all mitocarta genes that are present in TxDb.Hsapiens.UCSC.hg38.knownGenex
...
map #map for mitocarta
#relist(flesh, skeleton), where flesh is a vector-like object and skeleton is a list-like object. Only the "shape" (i.e. the lengths of the individual list elements) of skeleton matters. Its exact content is ignored.
```
...
Note that coding and UTR regions may not be present for all transcripts specified in map. Consequently, the subset results will not be the same length. This length discrepancy must be taken into account when re-listing the final results by gene.

```{r}
#Note that we rewrite the variables we created above with subsets based on txid in map
txid <- unlist(map, use.names=FALSE)
cds <- cds[names(cds) %in% txid] #take only cds whose names are in txid
...
length(txid) ## all possible transcripts corresponding to map
length(cds)
length(threeUTR)
length(fiveUTR)
cds
...
Not sure what the difference between extractTranscriptSeqs and getSeqs

This function retrieves the sequences for an any GRanges or GRangesList (i.e., not just 
...
cds_seqs <- extractTranscriptSeqs(hs_gm, cds) # returns a DNAStringSet instance of length 5999
# Not sure what's the advantage of using this vs. 
test_seq <- getSeq(hs_gm, cds) #returns DNAStringSetList of length 5999
test_seq
test_seq[[1]] ##hmmmm, this gives me a bunch of short sequences
#The return values are DNAStringSet objects.
```

#### Collecting the coding and UTR regions into groups by gene id. 

We use the relist function to group the sequences of a DNAStringSet object into a DNAStringSetList object, based on the specified skeleton argument. A simple form of skeleton is to use a partitioning object that we make by specifying the size of each partition. The partitioning objects are different for each type of region because not all transcripts had a coding or 3’ or 5’ UTR region defined.

```{r}
lst3 <- relist(threeUTR_seqs, PartitioningByWidth(sum(map %in% names(threeUTR))))
# sum(map %in% names(threeUTR)) how many sequences in this category
...
tryme <- unlist(lstc, use.names=TRUE)
writeXStringSet(tryme, file="mitocarda_cds.seq", format="fasta")
```

The table of element lengths shows how many genes have each number of transcripts.
...
The lists of DNA sequences all have the same length as map but one or more of the 
element lengths may be zero. This would indicate that data were not available for 
that gene. The tables below show that there was at least 1 coding region available 
for all genes (i.e., none of the element lengths are 0). However, both the 3’ and 5’ 
UTR results have element lengths of 0 which indicates no UTR data were available for 
that gene.
